...
const AddHomesClints = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.post("https://webi-bacend.onrender.com/posthomesclintsdata", formData); // Adjust the URL to your backend API
      Swal.fire({
        icon: "success",
        title: "Success",
        ...
        <div className="fixed top-0 right-0 left-0 z-50 flex justify-center items-center h-screen bg-black bg-opacity-40">
          <div className="relative p-4 w-full max-w-xl bg-white rounded-lg shadow-md">
            <div className="flex items-center justify-between p-4 border-b">
              <h2 className="text-xl font-semibold">Add Your Categories</h2>
              <button
                onClick={toggleModal}
                type="button"
            ...
            <div className="py-5 px-8">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <input
                  ...
                  placeholder="category ex:construction/qsr/hospitality"
                />
